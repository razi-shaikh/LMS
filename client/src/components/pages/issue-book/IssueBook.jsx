...
import { issueBook } from '@/api/issueReturn';
import { addAssignedToStudent } from '@/api/assignedStudents';
...
function IssueBook() {
  ...
  // Function to handle row selection of Books
  const onRowBook = () => {
  ...
  const useIssueBook = useMutation({
    mutationFn: issueBook,
    onSuccess: (data) => {
      console.log('Issue book successfully', data);
    },
    onError: (error) => {
      console.error('Error Issue book :', error);
    }
  })
  const useAssignedStudent = useMutation({
    mutationFn: addAssignedToStudent,
    onSuccess: (data) => {
      console.log('Issue book successfully', data);
    },
    onError: (error) => {
  ...
  const handelSubmit = async (e) => {
    e.preventDefault();
    try {
      await useIssueBook.mutateAsync({
        studentId: selectedStudent[0]?.student_id,
        bookId: selectedBook[0]?.id
      });

      await useAssignedStudent.mutateAsync({
        studentId: selectedStudent[0]?.student_id,
        bookId: selectedBook[0]?.id
      });
    } catch (error) {
  ...
  return (
    <div className='flex justify-center flex-col items-center'>
      <h2 className='text-center text-3xl my-3 tracking-wider uppercase font-bold'>Issue Book To Student</h2>
      <form className="w-full max-w-lg"
        ...
        <div className="flex flex-wrap -mx-3 my-6 gap-4">
          ...
          <div className="w-full px-3">
            ...
            <Select>
              <SelectTrigger className="w-full">
                <SelectValue placeholder={selectedStudent && selectedStudent[0]?.firstName || "Select a Book"} />
